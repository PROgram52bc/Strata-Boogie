...
# SMACK compiles C `assume(e)` to `call __VERIFIER_assume(e)`, whose body is
# `assume e != $0` ($0 is the zero constant); the call prunes infeasible paths
# and constrains nondet branches. Stripping deletes the body, and under the
# body-evaluating call policies a body-less procedure is analysed as a no-op —
# a `free ensures` contract is dropped, not assumed at the call site — so the
# constraint would be lost either way. Inline it directly at each call site,
# the Strata-side analogue of run_pipeline's CBMC `-D__VERIFIER_assume` mapping.
_VERIFIER_ASSUME_CALL = re.compile(
    r'^(\s*)call\s+__VERIFIER_assume\s*\(\s*(.+?)\s*\)\s*;\s*$')


def inline_verifier_assume(content: str) -> str:
    out = []
    for line in content.split('\n'):
        m = _VERIFIER_ASSUME_CALL.match(line)
        if m:
            out.append(f'{m.group(1)}assume ({m.group(2)} != $0);')
        else:
            out.append(line)
    return '\n'.join(out)
...
def main():
    ...
    result = inline_verifier_assume(content)
    result = strip_prelude_implementations(result)
